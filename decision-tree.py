...
def predict_example(root, example):
    if root is None:
        return []
    if root.is_leaf:
        return [root]
    if example[root.attr_split_index] >= root.attr_split_value:
        next_traces = predict_example(root.upper_child, example)
    else:
        next_traces = predict_example(root.lower_child, example)
    next_traces.append(root)
    return next_traces


def predict(root, example, is_trace=False):
    traces = predict_example(root, example)
    if len(traces) > 0:
        final_classification = traces[0].classification
    else:
...
def print_disjunctive(node, dataset, dnf_string):
    ...
    if node.is_leaf:
        ...
        upper = dnf_string + str(dataset.attributes[node.attr_split_index]) + " >= " + str(
            node.attr_split_value) + " V "
...
def main():
    ...
    if len(args) < 2:
        print("You have input less than the minimum number of arguments. Go back and read README.txt and do it "
              "right next time!")
    elif args[1][-4:] != ".csv":
        ...
        if "-t" in args:
            ...
            if "--trace" in args:
                _example = testset.examples[0]
                result = predict(root, _example, is_trace=True)
                print("classification:", result["final_classification"])
                print("traces:")
                traces = result["classify_traces"]
                for i in range(len(traces)):
                    trace_node = traces[i]
                    print("*** Level ", i)
                    print("Is final: ", trace_node.is_leaf)
                    print("Split by attribute:", trace_node.attr_split_index)
                    print("Node split value: ", trace_node.attr_split_value)
                    if trace_node.attr_split_index:
                        print("Example attribute value:", _example[trace_node.attr_split_index])
                    print("Node classification: ", trace_node.classification)
